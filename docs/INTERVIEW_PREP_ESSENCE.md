...
问题：在原项目基础上，方案 B 到底补了什么？

思路：不要讲成“多加了几个文件”，而要讲成“把主流大模型应用层显式化了”。

方案：
- 方案 B 的核心不是推翻原有 workflow-first 架构，而是在现有知识治理系统上，补一层显式 `context assembly`、一层受控只读 `tool registry`，再加一层 ask runtime `guardrail`。
- 这样项目就从“有 workflow、approval、checkpoint 的知识治理系统”，升级成“有 workflow orchestration、context engineering、bounded tool calling、runtime guardrail 的 Agent 应用工程项目”。

项目代码落地：
- `Context Assembly`：[backend/app/context/assembly.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/context/assembly.py)、[backend/app/context/render.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/context/render.py)、[backend/app/context/safety.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/context/safety.py)
- `Tool Registry`：[backend/app/tools/registry.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/tools/registry.py)、[backend/app/tools/ask_tools.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/tools/ask_tools.py)
- `Guardrail`：[backend/app/guardrails/ask.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/guardrails/ask.py)
- ask 主链路接入点：[backend/app/services/ask.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/ask.py)

### 26. 方案 B 实际新增了什么链路

问题：方案 B 做完之后，`ASK_QA` 的链路和以前有什么本质区别？

思路：重点不是“调了工具”，而是“模型前后各多了一层可控边界”。

方案：
- 旧 ask 更接近：
  - `query -> retrieval -> answer generation`
- 新 ask 更接近：
  - `query -> retrieval -> context assembly -> tool selection(optional) -> tool execution(optional) -> re-assembly -> answer generation -> guardrail -> response`
- 关键变化有 4 个：
  - retrieval 结果不再直接拼 prompt，而是先组装成 `ContextBundle`
  - 模型不拥有任意工具执行权，只能在白名单里选只读工具
  - tool result 不是事实真相，只是新增证据
  - 最终输出前还有显式 guardrail 决定是否放行

项目代码落地：
- ask 两段式主流程：[backend/app/services/ask.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/ask.py)
- ask graph trace 元数据：[backend/app/graphs/ask_graph.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/ask_graph.py)
- 共享 contract：[backend/app/contracts/workflow.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/contracts/workflow.py)

### 27. 方案 B 最关键的安全边界

问题：为什么方案 B 做完以后，项目更像大厂会认可的 LLM 应用工程，而不是“把 tool calling 接上了而已”？

思路：面试官真正关心的是边界，不是热点名词。

方案：
- 只给 `ASK_QA` 开放 tool calling，不把高风险治理链路一起放开
- 工具默认只读，不允许模型直接写 Vault
- tool result 必须先过安全检查，命中可疑注入内容时会在生成前直接降级
- tool result 只有在“能锚定到当前 retrieval 证据”时才允许进入最终 grounded prompt
- `search_notes / list_pending_approvals` 这类工具不会直接把原始结果塞进最终回答上下文

trade-off：
- 收益：面试里能讲清楚“工具不是越自由越好，重点是可控”
- 代价：不会像开放式 ReAct 那样灵活，Agent 自主性更低
- 但对这个项目来说，这是对的，因为它主线是知识治理和高风险写回控制，不是追求 agent 自由度

项目代码落地：
- tool 权限边界：[backend/app/tools/registry.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/tools/registry.py)
- tool 回流过滤与锚定检查：[backend/app/services/ask.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/ask.py)
- prompt 注入过滤：[backend/app/context/safety.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/context/safety.py)
- ask guardrail 放行逻辑：[backend/app/guardrails/ask.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/guardrails/ask.py)
...
方案：
- 可以讲：
  - 项目已经具备显式 `context assembly`
  - 项目已经具备 bounded read-only `tool calling`
  - ask 已经具备 runtime `guardrail`
  - ingest / digest 已经复用 context assembly
- 不要乱吹：
  - 还不是 MCP 项目
  - 还不是多 Agent / supervisor 架构
  - 还不是开放式 ReAct loop
  - 还没有通用 memory 框架

面试一句话：
- 我是有意识地把项目收敛成 workflow-first、strongly-bounded 的 Agent 应用，而不是为了热点把它做成一个难解释、难控制的开放式 agent。

### 29. 方案 B 完成状态与验证结果

问题：如果面试官问“你怎么证明这个增量真的落地了，不是 PPT 设计”，怎么答？

思路：拿代码、测试、eval 三层证据说话。

方案：
- 设计文档已沉淀：
  - [2026-03-22-context-assembly-tool-registry-design.md](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/docs/superpowers/specs/2026-03-22-context-assembly-tool-registry-design.md)
- 实施计划已沉淀：
  - [2026-03-22-context-assembly-tool-registry.md](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/docs/superpowers/plans/2026-03-22-context-assembly-tool-registry.md)
- 代码已合并到 `main`
- 合并后验证结果：
  - backend 全量测试 `104` 个通过
  - offline eval `21/21` 通过
  - `tool_and_guardrail` 场景 `3/3` 通过

项目代码落地：
- 单测重点：[backend/tests/test_ask_workflow.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/tests/test_ask_workflow.py)、[backend/tests/test_context_assembly.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/tests/test_context_assembly.py)、[backend/tests/test_tool_registry.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/tests/test_tool_registry.py)、[backend/tests/test_ask_guardrails.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/tests/test_ask_guardrails.py)
- eval 回归：[eval/run_eval.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/eval/run_eval.py)、[eval/golden/ask_cases.json](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/eval/golden/ask_cases.json)

### 30. 方案 B 可直接背的 45 秒表达

问题：面试官问“你最近给这个项目做了什么重要迭代”，怎么答？

方案：

我在这个项目上做了一轮比较关键的方案 B 增量迭代，核心目标不是再加一个热点名词，而是把项目从“只有 workflow 和审批控制”的知识治理系统，升级成一个更完整的 LLM 应用工程项目。具体来说，我补了显式的 `context assembly`，让 retrieval 结果不再直接拼 prompt；补了一个只读、白名单式的 `tool registry`，只在 `ASK_QA` 里开放受控工具调用；再补了 ask runtime `guardrail`，确保可疑 tool result 不会直接进模型、非锚定证据不会污染最终 grounded prompt。这样我在面试里就能讲清楚，这个项目不只是 LangGraph workflow，还具备 context engineering、bounded tool calling 和 runtime safety 的工程边界。

### 31. 方案 B 高频追问

问题：如果面试官追问“为什么你没有把所有 workflow 都开放 tool calling”，怎么答？

高分思路：
- 因为不同链路风险等级不同。
- `ASK_QA` 是最低风险路径，适合先开放只读工具。
- `INGEST_STEWARD / DAILY_DIGEST` 带 proposal、approval、writeback 语义，主线更应该先保证 evidence 组织和安全控制，而不是放大模型自由度。

问题：如果面试官追问“tool result 里本身就有 prompt injection 怎么办”，怎么答？

高分思路：
- 不能等模型看完再补救。
- 正确做法是在 tool result 回流最终 prompt 之前先做 safety 检查。
- 如果命中可疑模式，直接降级为 retrieval-only 或 `possible_injection_detected`，而不是先喂给模型再靠后处理修正。
