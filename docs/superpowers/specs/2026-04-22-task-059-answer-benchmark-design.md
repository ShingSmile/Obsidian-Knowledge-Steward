...
- runs the real ask path against canonical cloud model `qwen3.6-max-preview`
...
5. run each `case x variant` through the real ask path under `qwen3.6-max-preview`
...
Real `qwen3.6-max-preview` runs remain manual.
