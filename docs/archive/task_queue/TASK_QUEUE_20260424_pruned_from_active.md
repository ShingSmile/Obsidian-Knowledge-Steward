# Archived Task Queue Entries - 2026-04-24 Active Backlog Prune

本文件保存 2026-04-24 从 `docs/TASK_QUEUE.md` 主文件移出的旧完成 / 吸收任务块。主 `TASK_QUEUE.md` 继续只保留活跃、计划中与最近 benchmark 主线相关任务。

## Removed Obsolete Entry

旧 `TASK-060`（人工校准、bad case taxonomy 与报告导出）已按用户判断从 active queue 删除，不再作为独立任务维护。删除前内容如下，保留仅用于历史追溯。

### TASK-060

- `task_id`: `TASK-060`
- `session_id`:
- `title`: 为 `ASK_QA` benchmark 补人工校准、bad case taxonomy 与报告导出
- `category`: `Eval`
- `priority`: `P2`
- `status`: `deferred`
- `scope`: `medium`
- `goal`: 在 `TASK-057` 到 `TASK-059` 的数据与指标稳定之后，再补人工复核、失败类型归档与可复述报告，把 benchmark 从“开发者自己看得懂”升级成“能支撑简历和面试表达”的证据层。
- `out_of_scope`:
  - 不直接重写整页简历
  - 不把报告扩成 Web dashboard
  - 不扩展 governance / digest 的人工标注体系
  - 不做多 annotator 平台化管理
- `acceptance_criteria`:
  - 新增人工 review rubric，至少覆盖 `正确 / 部分正确 / 过度推断 / 应该拒答 / no-hit` 等标签
  - 至少 `20` 条 ask benchmark case 完成人工复核，并结构化记录 review 结果
  - 形成稳定的 bad case taxonomy，至少区分：`missed_retrieval / noisy_context / unsupported_claim / over_conservative_downgrade / tool_misuse`
  - 导出一份 markdown 报告，包含 baseline / ablation 表格、关键 bad case、当前安全可对外表述的数字与不应夸大的结论
  - 至少一处面试沉淀文档同步引用该报告结果
- `depends_on`:
  - `TASK-057`
  - `TASK-058`
...
- `derived_tasks`:
  - `small: 视人工复核一致性情况，决定是否补第二位 reviewer 的双标流程`
  - `small: 为报告导出增加 csv/json sidecar，便于后续做图或复用到简历草稿`
- `notes`: 2026-04-18 的第二轮校准已将本任务从第一阶段主线中移出。原因不是它不重要，而是当前更紧迫的缺口仍是：`TASK-057` 到 `TASK-059` 尚未形成一套固定 `50` 条 query、固定 canonical provider/model、真实 `generated_answer` 路径的基础 benchmark。在这些基础证据跑稳之前，人工校准与报告导出不应继续阻塞主线。

## Archived Completed / Absorbed Tasks
...
- `task_id`: `TASK-049`
- `session_id`: `SES-20260406-02`
- `title`: 将工具调用从 prompt-based JSON 约定迁移到 Structured Tool Calling
- `category`: `Graph`
- `priority`: `P1`
- `status`: `completed`
- `scope`: `medium`
- `goal`: 将当前 ask 链路中 LLM 与工具的交互主路径从“system prompt 文本描述 + 模型返回裸 JSON + 手动 `json.loads` 解析”迁移为 OpenAI-compatible Structured Tool Calling：LLM 调用时通过 API `tools` 参数传入结构化工具定义，模型返回结构化 `tool_calls` 字段，由框架优先消费；对不支持 Structured Tool Calling 的 provider 保留 prompt-based fallback。现有 ToolSpec 中的 `input_schema` 可直接映射为 `tools[].function.parameters`，迁移后 structured transport 成为主路径。
- `out_of_scope`:
...
- `acceptance_criteria`:
  ...
  - 模型返回的 `tool_calls` 字段由主路径响应解析逻辑提取，替代当前纯 prompt-based `json.loads(raw_text)` 方式作为默认路径
  - prompt 中的工具描述文本和 `_parse_tool_call_decision()` 的手动 JSON 解析只保留为 fallback，不再是默认协议
...
- `notes`: `SES-20260406-02` 已完成本任务并合并到 `main`。当前 [backend/app/tools/registry.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/tools/registry.py) 已新增 `build_chat_completion_tools_for_workflow()`，把现有 `ToolSpec.name / purpose / input_schema` 直接映射为 OpenAI-compatible `tools` payload；[backend/app/services/ask.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/ask.py) 现已在 tool selection 主路径上优先发送 `tools + tool_choice=auto` 并解析结构化 `message.tool_calls`，provider 不支持或 structured payload 不可用时再回落到原 prompt-based JSON 路径。图级 ReAct 循环、`tool_node`、guardrail 与 `execute_tool_call()` 的 domain path 保持不变。验收期内已通过 `tests.test_ask_workflow + tests.test_tool_registry` 共 `49/49` backend tests，以及 `12/12` ask targeted eval case。上面的 `derived_tasks` 继续保留为不阻塞收口的 `small` 尾项。
