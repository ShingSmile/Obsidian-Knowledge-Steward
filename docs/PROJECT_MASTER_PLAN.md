...
当前版本 `v0.2.58` 已同步到 `TASK-053` 的完成态与 `TASK-055` 的路径 contract 收敛：ask 离线评估现已稳定输出 Faithfulness / Answer Relevancy / Context Precision / Context Recall 四维度面板，ask golden 已扩充到 5 条以上质量 case；同时 backend / plugin / persistence / retrieval 已统一采用 `vault-relative` note path contract，输入层兼容 vault 内真实绝对路径并在进入正式 contract 前立即归一化，`/vault/...` 只保留给历史迁移。默认下一 medium 前移到 `TASK-054`，随后回到 `TASK-049` 与已后移的 `TASK-031`、`TASK-032`。
...
- 当前恢复已不再只停留在 completed checkpoint 短路返回，`DAILY_DIGEST` 与 `INGEST_STEWARD` 已各有一条真实 waiting proposal 路径，且插件待审批收件箱已可加载真实 pending item；ask 仍未 proposal 化，`INGEST_STEWARD` 的多 note proposal 合并、向量索引 coverage 观测、写回成功但后端记账失败的跨会话恢复、scoped ingest 的增量 FTS 同步、rollback 成功后的 scoped ingest 刷新，以及 ingest / digest 场景化指标仍未完成。基于 `TASK-044`、`TASK-045`、`TASK-046`、`TASK-047`、`TASK-050`、`TASK-051`、`TASK-052`、`TASK-053` 与 `TASK-055` 的完成，若继续开发，`TASK-048` 仍应视为 umbrella，本身不再作为单个 `medium` 执行，默认先进入 `TASK-054`，然后依次进入 `TASK-049`、已后移的 `TASK-031`、`TASK-032`；`TASK-033` 已被 `TASK-050` / `TASK-052` 吸收。插件产品形态、后端最小启动控制、受限写回安全链路、post-writeback 索引一致性、最小 rollback、最小向量底座、hybrid retrieval，以及当前 ask / ingest / digest 的基础 tracing、ask 四维度离线评估与分场景 benchmark 已视为 interview-first 下限达成；pending inbox freshness、CLI scoped parity、新笔记治理 proposal 的风险分级 / patch 模板收敛、rollback diff 预览，以及 faithfulness 停用词 / allowlist 收敛继续后移。digest 主体当前仍是模板化摘要，不包含调度、写回草稿或 LLM 聚类。
...
5. 已有 22 条 golden set、离线 `run_eval.py`、结果文件、governance suite、最小 `quality_metrics` / `metric_overview`，以及 `question_answering / governance` 两套 benchmark、场景级 `core_metrics` 与最小 `failure_type_breakdown`；但 markdown 报告导出、demo 脚本、宿主级演示路径和更强 judge 仍未补齐，如果后续不继续扩展，评估闭环仍会停在“能解释链路差异，但还不够易展示、也不够强语义评审”的层级。
...
- 下一阶段的重点不再是“继续补 P0 底座”或“再补 interview-first 保留 P1”，因为 `TASK-040`、`TASK-041`、`TASK-042`、`TASK-045`、`TASK-046`、`TASK-047`、`TASK-050`、`TASK-051`、`TASK-052`、`TASK-053` 与 `TASK-055` 都已完成；approval control-plane 到真实写回 side-effect plane 的最小闭环、post-writeback 索引一致性、最小 rollback、插件侧后端启动控制、向量 / hybrid retrieval、分场景 benchmark、ask 上下文装配层、graph-only ReAct 循环、共享 claim-level semantic faithfulness core、ask / digest runtime semantic gate，以及统一的 vault-relative path contract 都已接通。若继续开发，`TASK-048` 应作为 umbrella 看待，默认先进入 `TASK-054`，然后依次进入 `TASK-049`、`TASK-031`、`TASK-032`；`TASK-033` 已被 `TASK-050` / `TASK-052` 吸收。
...
| 优先级 | 补齐项 | 原因 | 不做会怎样 |
| --- | --- | --- | --- |
| P1 | 执行 `TASK-054`：完成 `TASK-048` umbrella 下剩余的 ingest / digest 评估扩面 | ask 四维度评估已由 `TASK-053` 收口，但 ingest / digest 场景指标、golden 扩面与阈值校准仍未完成 | 面试里仍会被继续追问“runtime 已接门之后，如何证明摘要覆盖充分、proposal rationale 忠实、跨链路质量没有盲区”，线上也仍可能放过 coverage 不足或 rationale 偏离 |
...
| Risk | 当前 `INGEST_STEWARD` 已接入 retrieval-backed `analysis_result` 与 cross-note evidence，ask 与治理也已共享同一条 hybrid candidate 底座，`TASK-040` 也已把 22 条 baseline 切成 `question_answering / governance` 两套 benchmark；但向量侧仍只有 best-effort 写入、最小禁用语义和固定 RRF 融合，没有 coverage 观测、branch 调试快照或 rerank，评估侧也还没有 markdown 报告导出、宿主级 benchmark 消费视图或更强 judge。 |
| Open Question | 首版云模型 provider 具体选哪家；本地 fallback 采用 Ollama 还是其他本地推理方式。 |
| Next Action | `TASK-030`、`TASK-034`、`TASK-035`、`TASK-036`、`TASK-037`、`TASK-038`、`TASK-039`、`TASK-040`、`TASK-041`、`TASK-042`、`TASK-045`、`TASK-046`、`TASK-047`、`TASK-050`、`TASK-051`、`TASK-052`、`TASK-053` 与 `TASK-055` 已完成；interview-first 保留主线已收口。若继续推进 medium，`TASK-048` 应视为 umbrella，默认进入 `TASK-054`，然后依次进入 `TASK-049`、已后移的 `TASK-031`、`TASK-032`；`TASK-033` 已被 `TASK-050` / `TASK-052` 吸收，其余 small 派生项继续后移。 |
...
| Next Action | 在 pending inbox、scoped ingest、首条 ingest proposal、ask graph-only ReAct 循环、post-writeback scoped reindex、retrieval-backed analyze、插件 backend runtime 控制、最小向量写入 / 检索、hybrid retrieval、分场景 benchmark、最小 rollback、统一 workflow contract、`TASK-047` 的四阶段上下文装配管线、`TASK-050` 的 ask runtime faithfulness 首刀、`TASK-051` 的共享 claim-level semantic faithfulness core、`TASK-052` 的 ask / digest runtime semantic gate、`TASK-053` 的 ask 四维度离线评估与 `TASK-055` 的 vault-relative path contract 都已落地的基础上，当前 interview-first 保留主线已收口；若继续推进 medium，`TASK-048` 应视为 umbrella，默认先进入 `TASK-054`，然后依次进入 `TASK-049`、`TASK-031`、`TASK-032`，`TASK-033` 已被 `TASK-050` / `TASK-052` 吸收，`TASK-041`、`TASK-046` 与 `TASK-047` 的既有 `small` 派生项继续后移。 |
...
1. 先用 22 条样本把 ask / governance / digest / resume-writeback 四类核心场景打成统一 baseline，再进一步收敛成 `question_answering / governance` 两套 benchmark，不强行追求一次性做满 benchmark 平台。
...
| Next Action | 在 22 条样本、governance suite、`schema_version=1.3`、`quality_metrics`、`metric_overview`、`benchmark_overview`、最小 `failure_type_breakdown`、rollback、统一 workflow contract、`TASK-047` 的 ask 装配回归、`TASK-046` 的 graph-only ReAct 循环、`TASK-050` 的 ask runtime faithfulness 首刀、`TASK-051` 的共享 claim-level semantic faithfulness core、`TASK-052` 的 ask / digest runtime semantic gate 与 `TASK-053` 的 ask 四维度离线基线都已补齐的基础上，评估侧继续是默认 medium 主线；若继续补评估能力，则优先执行 `TASK-054`，随后依次推进 `TASK-049`、`TASK-031` 与 `TASK-032`，`TASK-033` 已被 `TASK-050` / `TASK-052` 吸收。 |
...
| generated answer / digest 仍可能质量不足 | provider 虽然给出合法编号或模板化摘要，但结论仍可能超出证据、过度概括或覆盖不足 | ask / digest 可信度、演示可信度 | 人工抽样、golden set、共享 claim-level faithfulness report / runtime_faithfulness trace | 当前 ask 已完成四维度离线评估并扩充 ask golden，ask / digest runtime 也已接入 shared semantic gate；后续由 `TASK-054` 继续补齐 ingest / digest 场景指标、golden 扩面与阈值校准 | 是 |
