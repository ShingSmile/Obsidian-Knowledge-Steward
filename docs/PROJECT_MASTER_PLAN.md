...
| 当前版本号 | v0.2.47 |
| 最近更新时间 | 2026-03-18 CST |
| 当前开发阶段 | 基线验证已完成 / 索引预处理接入 / SQLite schema、ingest pipeline、FTS5 检索、metadata filter、最小 ask 引用式返回、ask 路径程序级 citation 对齐校验、ask_graph、ingest_graph、digest_graph、ask/ingest/digest runtime trace 的 JSONL + SQLite `run_trace` 聚合、SQLite `workflow_checkpoint` 与 ask/ingest/digest `thread_id` 恢复协议已落地 / `proposal`、`patch_ops`、`audit_log` 的 SQLite schema、`resume_workflow` 审批恢复控制面、插件审批面板 / diff 预览骨架与 `DAILY_DIGEST`、`INGEST_STEWARD` 的真实 `proposal + waiting_for_approval checkpoint`、插件真实上下文注入、插件侧受限写回执行器、`before_hash` 校验与 `writeback_result` 回传、写回成功后的 best-effort scoped ingest 索引刷新及结构化 `post_writeback_sync` 降级结果，以及 pending proposal 收件箱 / `GET /workflows/pending-approvals` 已落地 / 写回侧现已补齐最小 rollback：插件本地在真实写回前抓取 `LocalRollbackContext`，并通过 `POST /workflows/rollback` 把 rollback 成功 / 失败结果写入 append-only `audit_log`，且仅在成功 rollback 时更新 checkpoint；当前 rollback 仍刻意限制在当前插件会话内、命中精确 `after_hash` 的保守恢复边界 / `INGEST_STEWARD` 现已补齐 retrieval-backed analyze：可返回结构化 `analysis_result`、`retrieval_queries`、`related_candidates`，并在 proposal evidence 中引用已召回的旧笔记上下文，且显式排除 target note 自证 / 插件现已补齐 backend runtime 控制：支持用户提供启动命令后的最小本地自启、`/health` readiness probe、启动中 / 已就绪 / 不可用 / 启动失败状态展示、最近错误与输出尾日志，以及可选 auto-start on load / retrieval 底座现已补齐基于 SQLite `chunk_embedding` 的最小 embedding 写入与 `sqlite_vector` 检索入口，并进一步新增 `backend/app/retrieval/hybrid.py`：使用 RRF 融合 FTS + 向量检索、统一 metadata filter fallback / 去重语义，并让 ask 与 `INGEST_STEWARD` 的 related retrieve 共同消费 hybrid candidate；向量不可用时稳定退回 FTS / 离线 `eval/run_eval.py` 已升级到 `schema_version=1.3`，golden set 扩到 18 条，覆盖 ask / governance / digest / resume-writeback 四类核心场景，并新增最小 `quality_metrics`、全局 `metric_overview`、`question_answering / governance` 两套 `benchmark_overview`、场景级 `core_metrics` 与最小 `failure_type_breakdown` / 统一 workflow runtime contract 现已收口：`backend/app/graphs/runtime.py` 提供共享 `WorkflowGraphExecution`、基础 workflow state 与 runtime trace hook 组装，`backend/app/main.py` 的 `/workflows/invoke` 已改为 handler registry + 统一 invoker / outcome / response builder / `TASK-030`、`TASK-034`、`TASK-035`、`TASK-036`、`TASK-037`、`TASK-038`、`TASK-039`、`TASK-040`、`TASK-041`、`TASK-042` 已完成；interview-first 保留主线已全部收口，若继续开发默认回到 `TASK-031`、`TASK-032`、`TASK-033`；pending inbox freshness、CLI scoped parity、新笔记治理 proposal 的风险分级 / patch 模板收敛、rollback diff 预览、post-rollback scoped ingest，以及 groundedness 停用词 / allowlist 收敛继续后移 |
| 当前负责人 | qingfeng Qi |
| 文档定位 | 这是项目唯一主设计文档，后续所有实现、评估、演示与重构都必须同步维护本文件。 |

### 0.0.1 当前执行优先级约束

- 虽然后续所有具体实现、状态判断和接口边界都以“主文档 + 当前代码事实”为执行基准，但项目的 interview-first north star 仍然来自 [Obsidian Knowledge Steward 项目初步实现指南.md](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/Obsidian%20Knowledge%20Steward%20项目初步实现指南.md)。
- 这意味着：当前 interview-first 主线已经收口；后续会话不能只因为某个优化项“现在看起来顺手”就重开新路线，若继续开发默认按任务队列回到已后移的 medium backlog。
- 此前保留的三个 P1：`TASK-040`、`TASK-041`、`TASK-042` 现已全部完成；若继续推进 medium，默认顺序为：
  - `TASK-031`
  - `TASK-032`
  - `TASK-033`
- 除上述范围外，其他优化项即使有价值，也应默认后移，除非后续文档治理任务再次显式重排队列。
...
| v0.2.7 | 2026-03-12 | qingfeng Qi | 在“面试问答库”章节前新增统一角色设定，明确后续所有面试相关输出必须使用大厂资深大模型应用工程师兼高级面试官视角。 |
...
| v0.2.38 | 2026-03-17 | qingfeng Qi | 继续补全 `TASK-034` 的面试叙事同步：在第 13 节新增“为什么当前不继续推进 `TASK-031` 到 `TASK-033`”与“为什么基础 tracing 不再单独立 P0”两条关键追问，防止路线回正后面试口径仍停留在旧优先级。 |
...
- 文档旧描述在第 3 节 Gap Analysis 中仍写“缺少任何源码和可执行环境”，这与当前已经存在的 `backend/`、`plugin/`、`README.md` 和 conda 自检结果不一致，本次已修正为“缺少完整业务功能链路”。
- 文档旧描述在第 4 节系统组件划分中仍将插件层和后端层标记为“未实现”，但代码中已经有最小插件入口、后端 FastAPI 入口与协议模型，本次已修正为“基线已落地 / 部分实现”。
- 文档旧描述在第 9 节仍把 Metadata 解析层、插件入口层、配置层写成“当前无”，但当前已有 `backend/app/indexing/parser.py`、`plugin/src/main.ts`、`backend/app/config.py` 等实现，本次已修正。
- 文档旧描述在第 3、4、9 节仍把检索层写成“未实现”或“当前无”，但当前已有 `backend/app/retrieval/sqlite_fts.py` 与 `backend/app/contracts/workflow.py` 支撑 FTS5、metadata filter、标准 candidate 输出和 fallback，本次已修正。
- 文档旧描述在第 0、3、9、10 节仍把“最小 ask 引用式返回”写成待开始、或把 `/workflows/invoke` 的 `ask_qa` 写成占位响应，但当前代码已能返回 `ask_result`、citation 和 retrieval/model fallback，本次已修正。
- 文档旧描述在第 0、3、4、9、10 节仍把 `ask_graph` 写成“待开始”或把 LangGraph 工作流层描述成“仅有 `state.py`”，但当前代码已经新增 [backend/app/graphs/ask_graph.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/ask_graph.py) 并让 ask 真实走进 graph 入口，本次已修正。
- 文档旧描述在第 0、3、4、8、9、10 节仍把持久化 trace 写成“待开始”或“仅存在内存态 hook”，但当前代码已经新增 [backend/app/observability/runtime_trace.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/observability/runtime_trace.py) 并让 ask graph 默认落地 JSONL trace，本次已修正。
- 文档旧描述在第 0、3、4、8、9、10 节仍把 SQLite trace 聚合写成“待开始”，但当前代码已经在 [backend/app/indexing/store.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/indexing/store.py) 中新增 `run_trace` 表，并在 [backend/app/observability/runtime_trace.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/observability/runtime_trace.py) 中支持按 `run_id` / `thread_id` 的最小查询，本次已修正。
- 文档旧描述在第 0、3、4、9、10 节仍把 `ingest_graph` 与 `/workflows/invoke` 的 `INGEST_STEWARD` 写成待开始或占位响应，但当前代码已经新增 [backend/app/graphs/ingest_graph.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/ingest_graph.py) 并返回结构化 `ingest_result`，本次已修正。
- README 与主文档旧描述仍把 `TASK-026` 写成“下一步待做”，且没有同步说明 `INGEST_STEWARD` 已支持 scoped note ingest、vault 内路径校验和 scope-aware checkpoint 恢复 miss；这与 2026-03-17 的代码事实不一致，本次已修正。
- README 与主文档旧描述仍把 `TASK-027` 写成“下一步待做”，并把 `INGEST_STEWARD` proposal 能力描述成仅存在于设计图；但当前代码已经新增 [backend/app/services/ingest_proposal.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/ingest_proposal.py)，并让单 note scoped ingest 在显式 proposal 模式下真实进入 `waiting_for_approval`，本次已修正。
- README 与主文档旧描述仍把 `TASK-035` 写成“下一步待做”，并把 `INGEST_STEWARD` 的 related retrieve / analyze 节点表述成 hybrid + LLM 的设计态；但当前代码已经在 [backend/app/services/ingest_proposal.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/ingest_proposal.py)、[backend/app/graphs/ingest_graph.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/ingest_graph.py) 与 [backend/app/main.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/main.py) 中落地基于 SQLite FTS 的 retrieval-backed `analysis_result` 与 related evidence，本次已修正。
- README 与主文档旧描述仍把 `TASK-036` 写成“下一步待做”，并把插件能力表述成“只能手工分终端启动后端 + 被动 ping `/health`”；但当前代码已经在 [plugin/src/backend/runtime.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/backend/runtime.ts)、[plugin/src/main.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/main.ts)、[plugin/src/views/KnowledgeStewardView.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/views/KnowledgeStewardView.ts) 与 [plugin/src/settings.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/settings.ts) 中落地插件侧 backend runtime 控制、本地启动命令配置、`/health` readiness probe、状态展示与失败降级提示，本次已修正。
- README 与主文档旧描述仍把 `TASK-037` 写成“下一步待做”，并把 retrieval 底座表述成“当前只有 FTS + metadata filter”；但当前代码已经新增 [backend/app/retrieval/embeddings.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/retrieval/embeddings.py)、[backend/app/retrieval/sqlite_vector.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/retrieval/sqlite_vector.py)，并在 [backend/app/indexing/store.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/indexing/store.py) 中新增 `chunk_embedding` schema、在 [backend/app/indexing/ingest.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/indexing/ingest.py) 中补齐最小 embedding 写入，本次已修正。
- 文档旧描述在第 0、3、4、9、10 节仍默认 runtime trace 只覆盖 ask 路径，但当前代码已经让 ingest graph 复用 [backend/app/observability/runtime_trace.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/observability/runtime_trace.py) 的 JSONL + SQLite `run_trace` sink，本次已修正。
- 文档旧描述在第 0、3、4、5、9、10 节仍把 SQLite checkpoint 与恢复协议写成“未开始”，但当前代码已经新增 [backend/app/graphs/checkpoint.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/checkpoint.py)、在 [backend/app/indexing/store.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/indexing/store.py) 中新增 `workflow_checkpoint` 表，并让 ask / ingest 支持 `resume_from_checkpoint=true` + 显式 `thread_id` 的最小恢复，本次已修正。
- 文档旧描述在第 0、3、4、5、9、10 节仍把 `digest_graph` 与 `/workflows/invoke` 的 `daily_digest` 写成“未开始”或“占位响应”，但当前代码已经新增 [backend/app/graphs/digest_graph.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/digest_graph.py) 与 [backend/app/services/digest.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/digest.py)，并让 `daily_digest` 返回最小结构化 `digest_result`、`source_notes` 与安全 fallback，本次已修正。
- 文档旧描述在第 0、3、4、10 节仍把 `proposal` / `patch_ops` schema 与 `audit_log` 表写成“待开始”或“未实现”，但当前代码已经在 [backend/app/indexing/store.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/indexing/store.py) 中新增 `proposal`、`proposal_evidence`、`patch_op` 与 append-only `audit_log` 表，并补齐最小持久化 helper 与约束测试，本次已修正。
- 文档旧描述在第 0、3、4、5、9、10 节仍把 `resume_workflow`、审批恢复控制面和 `workflow_checkpoint.waiting_for_approval` 写成“未开始”或“仅规划中”，但当前代码已经新增 [backend/app/services/resume_workflow.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/resume_workflow.py)、[backend/app/main.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/main.py) 中的 `POST /workflows/resume`，并在 [backend/app/graphs/checkpoint.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/checkpoint.py) 与 [backend/app/indexing/store.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/indexing/store.py) 中补齐 `waiting_for_approval` 状态与 `v6` 迁移，本次已修正。
- 文档旧描述在第 0、3、4、9、10 节仍把插件审批 UI / diff 面板写成“待开始”，但当前代码已经在 [plugin/src/views/KnowledgeStewardView.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/views/KnowledgeStewardView.ts) 中补齐最小审批面板、risk / evidence / patch preview 与 approve / reject 交互，并在 [plugin/src/main.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/main.ts) 中增加本地 demo proposal 入口，本次已修正。
- 文档旧描述在第 0、3、4、9、10 节仍把“真实 proposal 注入”写成待开始，但当前代码已经让 [backend/app/graphs/digest_graph.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/digest_graph.py) 在显式 proposal 模式下真实产出 `proposal + waiting_for_approval checkpoint`，并让 [plugin/src/main.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/main.ts) 中的 `Load daily digest approval` 命令把真实上下文注入审批面板，本次已修正。
- 文档旧描述在第 0、3、4、9、10 节仍把“待审批列表 / 收件箱”写成未实现，但当前代码已经在 [backend/app/main.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/main.py) 中新增 `GET /workflows/pending-approvals`，并让 [plugin/src/views/KnowledgeStewardView.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/views/KnowledgeStewardView.ts) 渲染最小 pending inbox、空态 / 错误 / stale proposal 提示与真实 proposal 加载入口，本次已修正。
- 文档旧描述在第 0、3、4、9、10 节仍把插件侧受限写回执行器、`before_hash` 校验与真实 `writeback_result` 回传写成“未开始”，但当前代码已经新增 [plugin/src/writeback/applyProposalWriteback.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/writeback/applyProposalWriteback.ts) 与 [plugin/src/writeback/helpers.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/writeback/helpers.ts)，并让 [plugin/src/views/KnowledgeStewardView.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/views/KnowledgeStewardView.ts) 在 approval 时先本地执行受限写回、再把真实 `writeback_result` 回传给 [backend/app/services/resume_workflow.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/services/resume_workflow.py)，本次已修正。
- README 与主文档旧描述仍把 `TASK-042` 写成“下一步待做”，并把 `/workflows/invoke` 表述成三条链路各自分叉的入口；但当前代码已经在 [backend/app/graphs/runtime.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/runtime.py) 中收口共享 `WorkflowGraphExecution`、基础 workflow state 与 runtime trace hook，并在 [backend/app/main.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/main.py) 中改为 handler registry + 统一 invoker / outcome / response builder，本次已修正。
- 文档旧描述在第 3.4、4.1、9.1、9.2 节仍把 Vault 读写层写成“无 / 未实现”或把插件层能力描述成仅支持 panel / health 基线，但当前代码已经有 [plugin/src/writeback/applyProposalWriteback.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/writeback/applyProposalWriteback.ts) 的最小写回执行边界，因此本次已修正。
- 文档旧描述在第 3.2、3.3、3.4、3.5、9.11、10.2.6 节仍把 eval 写成“目录空壳”或把 `eval/run_eval.py` 写成未开始，但当前代码已经新增 [eval/run_eval.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/eval/run_eval.py)、[eval/golden/ask_cases.json](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/eval/golden/ask_cases.json)、[eval/golden/digest_cases.json](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/eval/golden/digest_cases.json) 与 [backend/tests/test_eval_runner.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/tests/test_eval_runner.py)，并已能产出 `eval/results/*.json`，本次已修正。
- [eval/README.md](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/eval/README.md) 旧描述仍写“首版直接从 `sample_vault/` 抽 30 条样本”，但当前真实状态已经是 18 条可运行 golden case、governance suite、`schema_version=1.3`、`benchmark_overview` 与稳定结果文件；本次主文档已按最新代码事实同步，`eval/README.md` 本身仍待后续文档收尾时补齐。
- README 旧描述仍把 `digest` 写成“仍未实现”、把 proposal / 审批写成“未开始”，这与当前 `TASK-022` 完成后的代码事实不一致，本次已修正。
- 文档旧描述在第 13 节仍把模型路线表述成“本地优先”，这与当前已确认的“云优先、本地兼容”不一致，本次已修正。
- 本次代码改动没有改变双端架构、后端本地部署边界和 LangGraph 作为工作流中枢的总体设计；变化主要发生在“实现状态”和“风险暴露面”，不是架构翻新。
- 原第 10.5 节把会话任务明细直接耦合在主文档中，不利于后续持续维护；本次已迁移到 `docs/TASK_QUEUE.md`，主文档只保留规则与引用。
- 根 README 曾默认使用 `backend/.venv`，但当前唯一有依赖事实支撑的运行环境实际上是工作区本地 conda prefix；本次已收敛为 `backend/environment.yml` + `python -m app.main`，并明确 `backend/.venv` 不再作为推荐入口。

### 0.2 当前事实边界

- 项目的起点确实是“只有实现指南的空仓”，但当前已经进入“基线骨架已落地”的阶段：仓库内已有 `backend/`、`plugin/`、`eval/`、`README.md`、`sample_vault/` 和 `references/ObsidianRAG/`。
...
- 当前已新增 `backend/app/indexing/store.py`、`backend/app/indexing/ingest.py`、`backend/app/retrieval/sqlite_fts.py`、`backend/app/retrieval/embeddings.py`、`backend/app/retrieval/sqlite_vector.py`、`backend/app/retrieval/hybrid.py`、`backend/app/services/ask.py`、`backend/app/services/digest.py`、`backend/app/services/ingest_proposal.py`、`backend/app/services/resume_workflow.py`、`backend/app/services/rollback_workflow.py`、`backend/app/graphs/runtime.py`、`backend/app/graphs/checkpoint.py`、`backend/app/graphs/ask_graph.py`、`backend/app/graphs/ingest_graph.py`、`backend/app/graphs/digest_graph.py`、`backend/app/observability/runtime_trace.py` 与 retrieval / ask / ingest / digest / graph / observability contract，可通过 `python -m app.indexing.store` 初始化 SQLite `note/chunk`、`chunk_embedding`、`run_trace`、`workflow_checkpoint`、`proposal`、`proposal_evidence`、`patch_op` 与 `audit_log` schema，通过 `python -m app.indexing.ingest` 将 `sample_vault/` 全量写入数据库，并通过 `backend/app/indexing/ingest.py` 的共享能力或 `/workflows/invoke` 的 `INGEST_STEWARD` 对 vault 内单 note / note 列表执行 scoped ingest；当前 ingest 在 embedding provider 可用时还会 best-effort 写入 `chunk_embedding`，provider 不可用时会显式降级而不阻断 note/chunk 主索引。通过 `python -m app.retrieval.sqlite_fts` 可执行带 metadata filter、标准 candidate 输出和 fallback 的最小 FTS5 检索，也可通过 `python -m app.retrieval.sqlite_vector` 对当前 provider/model 下已落库的 `chunk_embedding` 执行最小向量检索；后者会在 provider 不可用或向量索引尚未就绪时返回显式禁用语义，而不是伪装成普通 no-hit。与此同时，`backend/app/retrieval/hybrid.py` 现已可用 RRF 融合 FTS 与向量检索，并把 metadata filter fallback、重复 chunk 去重和融合后 `retrieval_source` 统一收敛到同一入口；ask 与 `INGEST_STEWARD` 的 related retrieve 都已切到这条 hybrid candidate 底座，向量不可用时会稳定退回 FTS，而不是让主链路直接报错。与此同时，`/workflows/invoke` 的 `ask_qa`、`ingest_steward` 与 `daily_digest` 已分别进入最小 ask / ingest / digest graph，并在 [backend/app/main.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/main.py) 中通过 handler registry + 统一 invoker / outcome / response builder 收敛为同一入口控制面；[backend/app/graphs/runtime.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/graphs/runtime.py) 现已提供共享 `WorkflowGraphExecution`、基础 workflow state 与 runtime trace hook 组装，ask / ingest / digest wrapper 统一复用这套 contract。其中 `ingest_steward` 现已支持 `note_path / note_paths`、vault 内路径校验、稳定去重排序和 `note_paths` 级 checkpoint 恢复匹配，并可在显式 `metadata.approval_mode="proposal"` 下对单 note scoped ingest 额外产出 retrieval-backed `analysis_result`、在 hybrid related candidate 命中时以跨 note evidence 支撑最小治理 proposal、把 thread 级 checkpoint 推进到 `waiting_for_approval`，并通过 `/workflows/invoke` 直接把 `thread_id / proposal_id / proposal / ingest_result / analysis_result` 返回给插件审批链路；该 analyze 目前仍显式限制在单 note scoped ingest、保守的 `orphan_hint / duplicate_hint` 与已有索引上下文边界内，target note 自身会被排除出 related evidence，fresh DB 或冷索引下若缺少旧上下文则只会安全 fallback，不会硬造跨 note 证据。`daily_digest` 在显式 proposal 模式下也已能额外产出结构化 `proposal` 与 waiting checkpoint。与此同时，ask / ingest / digest runtime trace 仍默认双写到 `data/traces/ask_runtime.jsonl` 和 SQLite `run_trace`，最近一次 workflow 状态写入 SQLite `workflow_checkpoint`；`backend/app/indexing/store.py` 已提供 `save_proposal()`、`save_proposal_record()`、`load_proposal()`、`list_pending_approval_records()` 与 `append_audit_log_event()` 最小 helper，而 [backend/app/main.py](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/backend/app/main.py) 中的 `POST /workflows/resume`、`POST /workflows/rollback` 和 `GET /workflows/pending-approvals` 已能分别处理审批恢复、rollback 记账与待审批收件箱查询；其中 `POST /workflows/resume` 在 successful `writeback_result` 下会对 `proposal.target_note_path` 触发 best-effort scoped ingest，并通过结构化 `post_writeback_sync` 返回刷新成功 / 失败结果，而 `POST /workflows/rollback` 会把 rollback 成功 / 失败结果写入 append-only `audit_log`，并仅在成功 rollback 时把 `rollback_result` 写回 completed checkpoint。插件侧 [plugin/src/views/KnowledgeStewardView.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/views/KnowledgeStewardView.ts) 现已能展示 pending inbox、proposal 摘要、风险、证据、安全检查、patch preview，以及单独的 Backend Runtime 状态区块；在用户提供 `backendStartCommand` / `backendStartWorkingDirectory` 的前提下，插件可对本地后端执行最小启动控制、基于 `/health` 判定 ready、展示 `checking / starting / ready / unavailable / failed`、最近错误、退出码与最近输出，并在 approve 时通过 [plugin/src/writeback/applyProposalWriteback.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/writeback/applyProposalWriteback.ts) 对 `merge_frontmatter`（兼容历史别名 `frontmatter_merge`）与 `insert_under_heading` 两类 patch op 执行首版受限写回、校验 `before_hash`、生成 `writeback_result`，再回传给 `/workflows/resume`；approve 成功后若仍停留在当前插件会话内，view 还可基于同次本地写回前抓取的 `LocalRollbackContext` 触发最小 rollback，并在“本地 rollback 已成功、后端记账失败”时只允许重试同步，不会重复改文件。[plugin/src/main.ts](/Users/qi/PycharmProjects/Obsidian-Knowledge-Steward/plugin/src/main.ts) 已新增 `Start backend`、`Refresh pending approvals` 与 `Load daily digest approval` 命令，并可按设置触发 `auto-start on load`。现在 `eval/` 也已不再只是空目录：`eval/run_eval.py` 能读取 ask / digest / governance / resume golden case、为 `sample_vault` / deterministic fixture 自动建索引、seed waiting proposal、预热 hybrid 检索，并产出带时间戳的 `eval/results/*.json`；当前 18 条 case 已覆盖 retrieval-only、retrieval fallback、hybrid ask、citation bad case、合法 grounded answer、semantic overclaim 的 `unsupported_claim` 分桶、structured digest、governance waiting proposal / no-proposal fallback，以及 `resume_workflow` 的 reject / successful writeback / failed writeback 三类高风险场景。结果 schema 已升级到 `schema_version=1.3`，每条 case 都可输出最小 `quality_metrics`，全局结果会聚合 `metric_overview`，并进一步收敛为 `question_answering / governance` 两套 `benchmark_overview`、场景级 `core_metrics` 与最小 `failure_type_breakdown`。当前恢复已不再只停留在 completed checkpoint 短路返回，`DAILY_DIGEST` 与 `INGEST_STEWARD` 已各有一条真实 waiting proposal 路径，且插件待审批收件箱已可加载真实 pending item；ask 仍未 proposal 化，`INGEST_STEWARD` 的多 note proposal 合并、向量索引 coverage 观测、写回成功但后端记账失败的跨会话恢复、scoped ingest 的增量 FTS 同步、rollback 成功后的 scoped ingest 刷新，以及 ask runtime groundedness gate 仍未实现。基于 `TASK-042` 的完成，当前 interview-first 保留主线（`TASK-040`、`TASK-041`、`TASK-042`）已全部收口；若继续开发，默认回到已后移的 `TASK-031`、`TASK-032`、`TASK-033`。插件产品形态、后端最小启动控制、受限写回安全链路、post-writeback 索引一致性、最小 rollback、最小向量底座、hybrid retrieval，以及当前 ask / ingest / digest 的基础 tracing 与分场景 benchmark 已视为 interview-first 下限达成；pending inbox freshness、CLI scoped parity、新笔记治理 proposal 的风险分级 / patch 模板收敛、rollback diff 预览，以及 groundedness 停用词 / allowlist 收敛继续后移。digest 主体当前仍是模板化摘要，不包含调度、写回草稿或 LLM 聚类。
...
| 参考实现 | `./references/ObsidianRAG/` | 对照已有双端 RAG 项目的插件/后端/检索实现，筛选可复用模块与反例 | 高 |
...
- 已新增只读参考仓库 `references/ObsidianRAG/`，用于借鉴双端 RAG 的已实现模块。
...
| 后端基线 | `backend/app/main.py`、`backend/app/contracts/workflow.py`、`backend/app/graphs/state.py`、`backend/app/graphs/runtime.py`、`backend/app/graphs/checkpoint.py`、`backend/app/graphs/ask_graph.py`、`backend/app/graphs/ingest_graph.py`、`backend/app/graphs/digest_graph.py`、`backend/app/observability/runtime_trace.py`、`backend/app/services/sample_vault.py`、`backend/app/services/ask.py`、`backend/app/services/digest.py`、`backend/app/services/ingest_proposal.py`、`backend/app/services/resume_workflow.py` | 暴露 `/health`、`/workflows/invoke`、`/workflows/resume`、状态 schema、协议模型、样本统计、最小 ask / ingest / digest 服务与审批恢复控制面，以及 JSONL + SQLite `run_trace` + SQLite `workflow_checkpoint` 的最小观测与恢复骨架 | 基线 + 最小 ask / ingest / digest graph / trace / checkpoint / approval resume，并已打通 `DAILY_DIGEST` 与 `INGEST_STEWARD` 的真实 proposal / waiting 路径，约 86% | 是 | 后续需扩展到 ask proposal、provider 健壮性、ingest richer evidence 与更强的 digest / writeback 质量控制 |
...
| 参考实现镜像 | `references/ObsidianRAG/` | 作为只读参考仓库分析可复用实现与反例 | 参考资产，非业务代码 | 选择性复用 | 不直接并入主代码 |
...
| 检索方案 | `backend/app/retrieval/sqlite_fts.py`、`backend/app/retrieval/embeddings.py`、`backend/app/retrieval/sqlite_vector.py`、`backend/app/retrieval/hybrid.py`、`backend/app/contracts/workflow.py`、`backend/app/services/ask.py`、`backend/app/services/ingest_proposal.py`、`sample_vault` 分析、`references/ObsidianRAG` 检索代码 | 半实现于 FTS5、metadata filter、`chunk_embedding` 持久化、最小向量检索、RRF hybrid candidate 输出、最小引用式 ask 与编号级 citation 对齐校验 | rerank、更强的语义级 groundedness 校验、向量 coverage / 性能治理与更多 hybrid 调试观测仍未实现 |
...
| 评估方案 | `eval/run_eval.py`、`eval/golden/*.json`、`eval/results/` | 半实现于 18 条 golden set、离线执行器、结果文件产出、governance suite、分场景 benchmark 与最小质量指标层 | 已能回归 ask / governance / digest / proposal / `resume_workflow` 代表场景，并输出最小 `faithfulness / relevancy / context` 指标、`question_answering / governance` 两套 benchmark、场景级 `core_metrics` 与最小 `failure_type_breakdown`，但尚未覆盖真实 Obsidian 宿主、markdown 报告导出与更强 judge |
...
| Vault 读写层 | `plugin/src/writeback/applyProposalWriteback.ts`、`plugin/src/writeback/helpers.ts` | 读取 note、frontmatter 处理、受限 patch 应用与 `before_hash` 校验 | 35% | 是 | 当前仅覆盖 `merge_frontmatter` / `insert_under_heading` 与最小 hash guard，后续需补更多受限 op、跨会话恢复与宿主级验证 |
...
5. 已有 18 条 golden set、离线 `run_eval.py`、结果文件、governance suite、最小 `quality_metrics` / `metric_overview`，以及 `question_answering / governance` 两套 benchmark、场景级 `core_metrics` 与最小 `failure_type_breakdown`；但 markdown 报告导出、demo 脚本、宿主级演示路径和更强 judge 仍未补齐，如果后续不继续扩展，评估闭环仍会停在“能解释链路差异，但还不够易展示、也不够强语义评审”的层级。
...
7. ask / ingest / digest graph 已能把最近一次 workflow 状态写入 SQLite `workflow_checkpoint`，并在显式携带 `resume_from_checkpoint=true` + `thread_id` 的情况下命中最近 checkpoint；completed checkpoint 会直接短路返回，避免重复 ask 调用、重复 scoped ingest 或重复 digest 生成。
...
- 下一阶段的重点不再是“继续补 P0 底座”或“再补 interview-first 保留 P1”，因为 `TASK-040`、`TASK-041`、`TASK-042` 都已完成；approval control-plane 到真实写回 side-effect plane 的最小闭环、post-writeback 索引一致性、最小 rollback、插件侧后端启动控制、向量 / hybrid retrieval、分场景 benchmark 与统一 workflow contract 都已接通。若继续开发，默认回到跨会话恢复、scoped ingest 的增量 FTS 同步与 ask runtime groundedness gate，即 `TASK-031`、`TASK-032`、`TASK-033`。
...
- ask 已补最小语义级 groundedness 离线评估，但仍缺更稳健的规则收敛、LLM judge 抽样辅助或线上 answer-citation semantic consistency gate。
...
| P2 | 执行 `TASK-033`：在 ask 主链路接入保守 groundedness gate 与安全降级 | ask 的 `unsupported_claim` 仍主要停留在离线 eval 暴露，没有进入 runtime 可信边界 | 面试里很容易被追问“离线看到了语义越界，那线上为什么还直接放行” |
...
| Risk | 当前 `INGEST_STEWARD` 已接入 retrieval-backed `analysis_result` 与 cross-note evidence，ask 与治理也已共享同一条 hybrid candidate 底座，`TASK-040` 也已把 18 条 baseline 切成 `question_answering / governance` 两套 benchmark；但向量侧仍只有 best-effort 写入、最小禁用语义和固定 RRF 融合，没有 coverage 观测、branch 调试快照或 rerank，评估侧也还没有 markdown 报告导出、宿主级 benchmark 消费视图或更强 judge。 |
| Open Question | 首版云模型 provider 具体选哪家；本地 fallback 采用 Ollama 还是其他本地推理方式。 |
| Next Action | `TASK-030`、`TASK-034`、`TASK-035`、`TASK-036`、`TASK-037`、`TASK-038`、`TASK-039`、`TASK-040`、`TASK-041`、`TASK-042` 已完成；interview-first 保留主线已收口。若继续推进 medium，默认回到已后移的 `TASK-031`、`TASK-032`、`TASK-033`，其余 small 派生项继续后移。 |
...
| 观测与评估层 | tracing、运行日志、错误日志、golden set、回归脚本、报告输出 | 不参与正常用户交互主链路 | `backend/app/observability/`、`eval/` | ask / ingest / digest graph 已有最小 trace hook，JSONL trace 与 SQLite `run_trace` 聚合已覆盖三条 graph，`eval/run_eval.py` 也已落 ask / digest / `resume_workflow` golden 回归与 ask groundedness 分桶；更完整 observability 模块、指标聚合与宿主级验证仍未实现 |
| 存储层 | SQLite、向量存储、checkpoint、JSONL trace、配置文件 | 不提供业务逻辑 | `data/`、`backend/data/` | SQLite `note/chunk`、`chunk_embedding`、`run_trace`、`workflow_checkpoint`、`proposal`、`patch_op` 与 `audit_log` 已落地；当前向量侧仍是 SQLite 内持久化与 JSON embedding，不是独立向量库，更完整持久化治理与性能优化仍未实现 |
...
| Next Action | 在 pending inbox、scoped ingest、首条 ingest proposal、ask groundedness eval、post-writeback scoped reindex、retrieval-backed analyze、插件 backend runtime 控制、最小向量写入 / 检索、hybrid retrieval、分场景 benchmark、最小 rollback 与统一 workflow contract 都已落地的基础上，当前 interview-first 保留主线已收口；若继续推进 medium，默认回到 `TASK-031`、`TASK-032`、`TASK-033`，`TASK-041` 的既有 `small` 派生项（rollback diff 预览、post-rollback scoped ingest）继续后移。 |
...
1. 先用 18 条样本把 ask / governance / digest / resume-writeback 四类核心场景打成统一 baseline，再进一步收敛成 `question_answering / governance` 两套 benchmark，不强行追求一次性做满 benchmark 平台。
...
3. 当前结果 schema 已升级到 `1.3`，除 workflow status、fallback 信号、citation / proposal 结构外，还会输出最小 `quality_metrics`、全局 `metric_overview`、`question_answering / governance` 两套 `benchmark_overview`、场景级 `core_metrics` 与最小 `failure_type_breakdown`。
...
| Next Action | 在 18 条样本、governance suite、`schema_version=1.3`、`quality_metrics`、`metric_overview`、`benchmark_overview`、最小 `failure_type_breakdown`、rollback 与统一 workflow contract 都已补齐的基础上，评估侧当前不再单独占主线；若继续补评估能力，则优先处理 `TASK-040` 已登记的 markdown 报告导出等 `small` 派生项，medium 主线默认回到 `TASK-031` 到 `TASK-033`。 |
...
- 当前版本是否稳定：部分实现。已支持按 `proposal.patch_ops` 执行受限写回，覆盖 `merge_frontmatter`（兼容历史别名 `frontmatter_merge`）与 `insert_under_heading` 两类操作，并在写回前校验 `before_hash`；但路径语义统一、跨会话恢复与更完整 patch 能力仍未完成。
...
- 关键类 / 函数 / 数据结构：`StewardState`、`invoke_checkpointed_linear_graph()`、`PersistedGraphCheckpoint`、`resume_workflow()`。
...
- 可替代实现：完全手工评估；只能短期过渡。
- 当前版本是否稳定：部分实现。已能执行最小 ask / digest / proposal / `resume_workflow` golden 回归、按 `case_id` 过滤执行并生成 `eval/results/*.json`；但样本规模、宿主级验证、指标聚合和长期样本治理仍未完成。
...
| 接入 SQLite checkpoint | `已完成（ask / ingest / digest 路径已落 workflow_checkpoint，与 waiting_for_approval 状态兼容的 v6 schema 也已补齐）` |
...
| generated answer 仍可能语义未严格 grounded | provider 虽然给出合法编号，但结论本身超出证据、过度概括或偷换表述 | ask 可信度、演示可信度 | 人工抽样、golden set、`ask_groundedness` / answer-citation consistency | 当前已对缺引用/越界编号自动退回 retrieval-only，并通过离线 groundedness eval 暴露 `unsupported_claim`；后续再评估是否引入线上 gate | 是 |
