...
当前版本 `v0.2.53` 对应 `TASK-047` 的完成态：ask 上下文装配层已从“去重 + 截断”升级为相关性过滤、来源多样性控制、结构化增强与加权预算分配四阶段质量控制管线，`ContextBundle` 现可输出 `source_notes / assembly_metadata`，prompt 也已可见 `source_note_title / position_hint`，citation 与 `retrieved_candidates` 继续只跟随 post-assembly 可见 evidence，因此 `TASK-047` 已收口。
...
- 下一阶段的重点不再是“继续补 P0 底座”或“再补 interview-first 保留 P1”，因为 `TASK-040`、`TASK-041`、`TASK-042`、`TASK-045` 与 `TASK-047` 都已完成；approval control-plane 到真实写回 side-effect plane 的最小闭环、post-writeback 索引一致性、最小 rollback、插件侧后端启动控制、向量 / hybrid retrieval、分场景 benchmark 与 ask 上下文装配层都已接通。若继续开发，默认先进入 `TASK-046`，在 ask 链路内部补 ReAct 多轮工具调用；其后再回到 `TASK-031`、`TASK-032`、`TASK-033`。
...
| 优先级 | 补齐项 | 原因 | 不做会怎样 |
| --- | --- | --- | --- |
| P1 | 执行 `TASK-046`：在 ask 链路内部引入 ReAct 多轮工具调用循环 | 当前 ask 已有更强的 post-assembly 上下文质量控制，但仍只允许单轮工具调用，复杂跨笔记问题仍可能受限于一次补查 | 面试里会被继续追问“检索质量提高了以后，为什么 ask 仍然只能查一次就回答”，复杂问答质量也会被单轮上限卡住 |
...
| Next Action | `TASK-030`、`TASK-034`、`TASK-035`、`TASK-036`、`TASK-037`、`TASK-038`、`TASK-039`、`TASK-040`、`TASK-041`、`TASK-042`、`TASK-045` 与 `TASK-047` 已完成；interview-first 保留主线已收口。若继续推进 medium，默认进入 `TASK-046`，其后回到已后移的 `TASK-031`、`TASK-032`、`TASK-033`，其余 small 派生项继续后移。 |
...
| 检索与索引层 | Markdown 解析、chunking、metadata schema、FTS/vector/hybrid/rerank | 不负责 UI、不负责审批决策 | `backend/app/indexing/`、`backend/app/retrieval/` | 解析器、SQLite schema、ingest、FTS、metadata filter、`chunk_embedding` 持久化、最小向量检索、RRF hybrid retrieval，以及四阶段 ask context assembly、`source_notes / assembly_metadata` 输出和 post-assembly citation 对齐校验都已落地；ask 的离线 groundedness / `unsupported_claim` 评估也已补齐，但 rerank/线上 groundedness gate、coverage 观测与性能治理仍未实现 |
...
| Next Action | 在 pending inbox、scoped ingest、首条 ingest proposal、ask groundedness eval、post-writeback scoped reindex、retrieval-backed analyze、插件 backend runtime 控制、最小向量写入 / 检索、hybrid retrieval、分场景 benchmark、最小 rollback、统一 workflow contract 与 `TASK-047` 的四阶段上下文装配管线都已落地的基础上，当前 interview-first 保留主线已收口；若继续推进 medium，默认先进入 `TASK-046`，其后回到 `TASK-031`、`TASK-032`、`TASK-033`，`TASK-041` 与 `TASK-047` 的既有 `small` 派生项继续后移。 |
...
| Next Action | 在 18 条样本、governance suite、`schema_version=1.3`、`quality_metrics`、`metric_overview`、`benchmark_overview`、最小 `failure_type_breakdown`、rollback、统一 workflow contract 与 `TASK-047` 的 ask 装配回归都已补齐的基础上，评估侧当前不再单独占主线；若继续补评估能力，则优先处理 `TASK-040` 已登记的 markdown 报告导出与 `TASK-047` 已登记的阈值离线 eval 等 `small` 派生项，medium 主线默认先进入 `TASK-046`，其后回到 `TASK-031` 到 `TASK-033`。 |
